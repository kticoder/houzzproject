...
class houzzararesimasync:
    ...
    def resimurlgetir(self):
        ...
        for i in range(self.resimbasi,self.resimson):
            ...
            konsol.print(f"[bold red] {self.kategorilinki}/p/{sayfa} sayfası indiriliyor. [/]")
    ...
    async def image_downloader(self,session,resimindir):
        for i in resimindir:
            async with session.get(i) as resp:
                self.resimsayisi += 1
                f = await aiofiles.open(f'{os.getcwd()}/HouzzAraResim/resim{self.resimsayisi}.jpg', mode='wb')
